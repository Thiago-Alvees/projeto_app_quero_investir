...
export default function PortfolioListScreen() {
  ...
  const loadData = useCallback(() => {
    ...
    (async () => {
      ...
      const [myPortfolios, publicPortfolios] = await Promise.all([
        loggedWithAccount ? listPortfolios() : Promise.resolve([]),
        listPublicPortfolios(),
      ]);

      if (!active) return;
      setHasAccountSession(loggedWithAccount);
      setTab((prev) => (prev === "MINE" && !loggedWithAccount ? "PUBLIC" : prev));
  ...
  const canCreate = tab === "MINE" && hasAccountSession;
  const requiresLoginForMine = tab === "MINE" && !hasAccountSession;
        ...
        <View style={styles.header}>
          <View style={styles.headerInfo}>
            ...
            <Text style={styles.catalogStatus}>
              Base: {source === "SNAPSHOT" ? "snapshot curado" : "fallback local"}
          ...
          {canCreate ? (
            ...
            <Pressable onPress={() => navigation.navigate("AccountTab")} style={styles.secondaryBtn}>
              <Text style={styles.secondaryBtnText}>Entrar</Text>
            </Pressable>
          ) : null}
        </View>
        ...
        {loading ? (
          ...
          <View style={styles.empty}>
            <Text style={styles.emptyTitle}>
              {requiresLoginForMine
                ? "Entre com sua conta para montar carteiras."
                : tab === "MINE"
            ...
            <Text style={styles.emptyText}>
              {requiresLoginForMine
                ? 'Você pode explorar carteiras públicas sem login. Para montar carteira, acesse "Conta".'
                : tab === "MINE"
                  ? 'Toque em "Nova" para montar sua carteira.'
                  : "Quando usuários marcarem uma carteira como pública, ela aparece aqui."}
            </Text>
            {requiresLoginForMine ? (
              <Pressable onPress={() => navigation.navigate("AccountTab")} style={styles.inlineBtn}>
                <Text style={styles.inlineBtnText}>Ir para Conta</Text>
              </Pressable>
            ) : null}
          </View>
        ) : (
                ...
                <Pressable
                  ...
                  onPress={() =>
                    navigation.navigate("PortfolioDetail", {
                      ...
                      readOnly: tab === "PUBLIC" || !hasAccountSession,
                    })
                  }
                >
...
const styles = StyleSheet.create({
  ...
  inlineBtnText: { color: "#111", fontWeight: "700", fontSize: 12 },
  list: { paddingTop: 14 },
  card: {
    backgroundColor: "#fff",
    borderRadius: 12,
